...
export default function SOPIntelligence() {
  ...
  const fetchHistory = async (uid, embId) => {
    try {
      const res = await fetch(
        `${API_RAG_URL}/global-user/history/${uid}/${embId}`
      );
      if (!res.ok) return;
  ...
  const handleClearHistory = async () => {
    if (
      !window.confirm(
        "Are you sure you want to clear your personal chat history?"
      )
    )
      return;
    try {
      await fetch(`${API_RAG_URL}/global-user/clear-history/${userId}`, {
        method: "POST",
      });
      setMessages([]);
  ...
  const handleClearCache = async () => {
    if (
      ...
      return;
    try {
      const res = await fetch(`${API_RAG_URL}/admin/clear-cache/global`, {
        method: "POST",
      });
  ...
  const handleGlobalSync = async () => {
    ...
    setSyncStatus({
      type: "info",
      message: "Initiating global sync pipeline...",
    });
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/sops/sync-rag`, {
        method: "POST",
      ...
      syncIntervalRef.current = setInterval(async () => {
        try {
          const statusRes = await fetch(
            `${API_BASE_URL}/api/sops/sync-status`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          ...
          setSyncStatus({
            type: "info",
            message: `Syncing: ${completedSoFar}/${initialTotal} processed (${stats.failed} failed)`,
          });

          // Stop polling when pending hits 0
          if (stats.pending === 0) {
            ...
            setSyncStatus({
              ...
              message:
                stats.failed > 0
                  ? `Sync completed with ${stats.failed} failures.`
                  : "Global Vector Database is now 100% up to date!",
            });
  ...
  const handleDeleteSingleApprovedQA = async (qaId) => {
    ...
    try {
      const res = await fetch(
        `${API_RAG_URL}/global-api/approved-qas/${qaId}`,
        { method: "DELETE" }
      );
      const data = await res.json();
  ...
  const handleDeleteApprovedQAs = async () => {
    const ok = window.confirm(
      "Delete ALL approved Q/A vectors?\n\nThis cannot be undone."
    );
    if (!ok) return;
    setApprovedQaLoading(true);
    try {
  ...
  const handleSyncApprovedQAs = async () => {
    ...
    try {
      const res = await fetch(`${API_RAG_URL}/global-api/sync-approved-qas`, {
        method: "POST",
      });
      const data = await res.json();
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSyncStatus({
          type: "success",
          message:
            "Answer approved and instantly cached to the global database.",
        });
        setTimeout(() => setSyncStatus({ type: null, message: "" }), 4000);
      } else {
        const errorData = await res.json();
        alert(
          `Failed to approve answer: ${errorData.detail || "Unknown error"}`
        );
      }
    } catch (error) {
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center h-full max-w-xl mx-auto px-4 text-center animate-in fade-in zoom-in duration-500">
      ...
      <p className="text-xs text-slate-500 mb-6">
        Ask any question and I will automatically route it to the right
        documentation to get you instant, verified answers.
      </p>
  ...
  return (
    <>
      <div className="flex h-full flex-col bg-slate-50 font-sans">
        ...
        {syncStatus.message && (
          <div
            className={`px-4 py-2 text-[11px] font-medium shrink-0 shadow-sm z-20 border-b flex flex-col items-center gap-1 ${
          ...
          >
            ...
            {isSyncing && syncProgress.total > 0 && (
              <div className="w-full max-w-md bg-slate-200 rounded-full h-1.5 mt-1 overflow-hidden">
                <div
                  className={`h-full transition-all duration-500 ${
                    syncProgress.failed > 0 ? "bg-amber-500" : "bg-blue-600"
                  }`}
                  style={{
                    width: `${
                              ...
                              <div className="flex flex-col gap-2 mt-2">
                                {msg.approvedOptions?.map((opt, idx) => (
                                  ...
                                  >
                                    ...
                                    <span className="text-slate-700 group-hover:text-emerald-800">
                                      {opt.question}
                                    </span>
                                  </button>
                                ))}
                                ...
                                <div className="flex items-center gap-2">
                                  <div className="text-[10px] font-medium text-slate-400 flex items-center gap-1">
                                    {msg.source?.includes("cache") && (
                                      <>
                                        <AlertCircle className="w-3 h-3" /> Fast
                                        Cache
                                      </>
                                    )}
                                  </div>
            ...
            <div className="w-full sm:w-80 md:w-[400px] shrink-0 border-l border-white/40 bg-slate-50/90 backdrop-blur-md h-full flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-30 absolute right-0 top-0 sm:relative transition-all duration-300">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-white/80 shadow-sm shrink-0">
                <h3 className="text-xs font-bold text-slate-700 flex items-center gap-2 uppercase tracking-wider">
                  <Terminal size={14} className="text-purple-600" /> RAG Debug
                  Data
                </h3>
                <button
                ...
                {!activeContextData ? (
                  <div className="flex flex-col items-center justify-center h-full text-center text-slate-400">
                    ...
                    <p className="text-[10px] mt-1">
                      Ask a question or click "View Source Data" on an existing
                      answer.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3 font-mono">
                    ...
                    <details
                      className="border border-slate-200 rounded-md bg-white/80 shadow-sm group"
                      open
                    >
                      <summary className="p-2.5 text-[11px] font-bold text-slate-700 cursor-pointer select-none group-open:border-b border-slate-100 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        <span className="flex items-center gap-1.5">
                          <Code size={13} className="text-orange-500" /> Final
                          LLM Context
                        </span>
                      </summary>
                      <div className="p-0 bg-slate-900/95 overflow-hidden rounded-b-md">
                        <pre className="text-[10px] text-emerald-400 p-3 max-h-[300px] overflow-y-auto whitespace-pre-wrap break-words leading-relaxed scrollbar-thin scrollbar-thumb-slate-700">
                          {activeContextData.final_llm_context ||
                            "No context assembled for LLM."}
                        </pre>
                      </div>
                    </details>
                    ...
                    <details className="border border-slate-200 rounded-md bg-white/80 shadow-sm group">
                      ...
                      <div className="p-2 bg-slate-50/50 max-h-[300px] overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-slate-300">
                        {activeContextData.reranked_search?.map((item, idx) => (
                          <div
                            key={idx}
                            className="border border-slate-200/60 p-2 rounded bg-white text-[10px]"
                          >
                            <div className="flex justify-between items-center mb-1.5 pb-1.5 border-b border-slate-100">
                              <span className="font-bold text-slate-600 bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded">
                    ...
                    <details className="border border-slate-200 rounded-md bg-white/80 shadow-sm group">
                      <summary className="p-2.5 text-[11px] font-bold text-slate-700 cursor-pointer select-none group-open:border-b border-slate-100 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        ...
                        <span className="text-[10px] text-slate-400 font-normal bg-slate-100 px-1.5 py-0.5 rounded-full">
                          {activeContextData.initial_search?.length || 0} chunks
                        </span>
                      </summary>
                      <div className="p-2 bg-slate-50/50 max-h-[300px] overflow-y-auto space-y-2 scrollbar-thin scrollbar-thumb-slate-300">
                        {activeContextData.initial_search?.map((item, idx) => (
                          ...
                          >
                            ...
                            <p className="whitespace-pre-wrap text-slate-500 leading-relaxed line-clamp-4 hover:line-clamp-none transition-all">
                              {item.text}
                            </p>
                          </div>
                        ))}
                    ...
                    <details className="border border-slate-200 rounded-md bg-white/80 shadow-sm group">
                      <summary className="p-2.5 text-[11px] font-bold text-slate-700 cursor-pointer select-none group-open:border-b border-slate-100 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        <span className="flex items-center gap-1.5">
                          ...
                          Extracted Parent Nodes
                        </span>
                        <span className="text-[10px] text-slate-400 font-normal bg-slate-100 px-1.5 py-0.5 rounded-full">
                          {activeContextData.parent_sections?.length || 0} nodes
                        </span>
                      </summary>
                      <div className="p-0 bg-slate-900/95 overflow-hidden rounded-b-md">
                        <pre className="text-[10px] text-sky-400 p-3 max-h-[300px] overflow-y-auto whitespace-pre-wrap break-words leading-relaxed scrollbar-thin scrollbar-thumb-slate-700">
                          {JSON.stringify(
                            activeContextData.parent_sections,
                            null,
                            2
                          )}
                        </pre>
                    ...
                    .map((row) => (
                      <tr key={row.id} className="border-b hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 align-top">
                          <button
                            onClick={() => handleDeleteSingleApprovedQA(row.id)}
